...
#CC Check Charge 0.1$
@userge.on_cmd("cd", about={
	'header': "CC Check Charge 0.1$",
	'usage': "{tr}cd [Input CC]\n"})
...
async def gen(message: Message):
	...
	try:
		async with userge.conversation(chat) as conv:
			try:
				await conv.send_message("/cd {}".format(replied))
...
#CC Charge $1
@userge.on_cmd("cg", about={
	'header': "CC Charge $1",
	'usage': "{tr}cg [Input CC]\n"})
...
async def bin(message: Message):
	...
	try:
		async with userge.conversation(chat) as conv:
			try:
				await conv.send_message("/cg {}".format(replied))
			except YouBlockedUser:
				await message.err(f"**{ERROR_MSG}**", del_in=5)
				return 
			msgs = await conv.get_response(timeout=30, mark_read=True)
...
#3DS
@userge.on_cmd("3d", about={
	'header': "3DS Check",
	'usage': "{tr}3d [Enter full card in format]\n"})
...
async def vbv(message: Message):
	...
	try:
		async with userge.conversation(chat) as conv:
			try:
				await conv.send_message("/3d {}".format(replied))
			except YouBlockedUser:
				await message.err(f"**{ERROR_MSG}**", del_in=5)
				return 
			msgs = await conv.get_response(timeout=30, mark_read=True)
